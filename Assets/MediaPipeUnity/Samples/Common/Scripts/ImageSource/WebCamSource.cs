...
#pragma warning restore IDE0025
    ...
    private IEnumerator Initialize()
    {
      ...
      {
        // 名前に "Logi" が含まれるカメラを検索
        var logiCamera = availableSources.FirstOrDefault(d => d.name.Contains("Logi"));

        // 名前に "USB" が含まれるカメラを検索
        var USBCamera = availableSources.FirstOrDefault(d => d.name.Contains("USB"));
        
        if (logiCamera.name != null) {
            webCamDevice = logiCamera; // 見つかったらLogiを使う
        } else if (USBCamera.name != null)
        {
            webCamDevice = USBCamera; // 見つかったらUSBを使う
        } else {
            webCamDevice = availableSources[0]; // 見つからなければ1番目のカメラ
        }
      }
    }
