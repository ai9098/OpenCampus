...
当初はPython版MediaPipeで取得したランドマーク情報をUnityへ送信する予定でしたが、
通信による遅延が発生したため、[MediaPipeUnityPlugin](https://github.com/homuler/MediaPipeUnityPlugin/releases)を利用してUnity内でランドマーク情報を直接取得する方式に変更しました。
これにより、通信による遅延を解消し、リアルタイムなジェスチャー認識を実現しました。
...
- 親指を判定に含むと誤判定が発生しやすいため、判定から除外した。
- ランドマーク座標から算出した手のひらや、人差し指に対して、エフェクトが追従するようにした。
- 木材・溶岩ブロックの二種類が存在し、木材には炎を、溶岩には水のエフェクトを当て続けることで破壊できるようにした。
- 2Dゲームのため奥行き情報をそのまま利用できないことから、MediaPipeに含まれるZ軸の情報を取り除き、エフェクトとカメラの距離を固定した。
